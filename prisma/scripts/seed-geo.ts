// Сидирование гео-блока: конфиги страниц всех зон + локальные цены (GeoProductData)
// Запуск: npx tsx prisma/scripts/seed-geo.ts
...
async function main() {
  ...
  // 1. Конфиги страниц для всех зон
  for (const zone of zones) {
    ...
    console.log(`  Конфиг geo:${zone.slug} — готов`);
  }

  // 2. Локальные цены для всех товаров с ценой во всех зонах
  const products = await prisma.product.findMany({
    where: { priceRetailBase: { not: null } },
    select: { id: true, name: true, priceRetailBase: true },
  });
  console.log(`Товаров с ценой: ${products.length}`);
  ...
  console.log(`GeoProductData: создано ${created}, обновлено ${updated}`);
}
