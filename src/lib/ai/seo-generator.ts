...
Твоя задача: написать авторитетный, глубоко структурированный лонгрид для блога pesok-metall.ru.

ПРАВИЛА:
1. Минимум 5000 знаков (символов) — это обязательное требование.
2. Структура: H1 заголовок, введение, 3-5 разделов H2, маркированные списки, таблицы, заключение.
3. Включи: практические расчёты, таблицы сравнения, текущие цены, локальные ссылки.
4. Стиль: живой, экспертный, без канцелярита. Для строителей, прорабов и застройщиков.
5. Органично вставь ключи: "купить ${product.name}", "доставка в ${inZone(zone.name)}", "ГОСТ", "цена за тонну".
6. Включи внутренние ссылки: /metall, /geo/${zone.slug}, /blog.
7. Добавь таблицу с ценами (HTML <table>).
8. Выдай ответ в формате JSON.

Формат ответа (JSON):
{
  "title": "Заголовок статьи (H1)",
  "content": "Полный HTML-текст статьи (h2, p, ul, li, table, strong). Минимум 5000 знаков!",
...
{
  "title": "Заголовок статьи (H1)",
  "content": "Полный текст статьи в HTML (используй h2, p, ul, li, table, strong). МИНИМУМ 5000 ЗНАКОВ!",
  "seoTitle": "SEO Title (60-70 знаков)",
  "seoDescription": "Meta Description (140-160 знаков)"
}`;
