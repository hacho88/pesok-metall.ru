...
/**
 * IdealAsset — премиальное превью товара для витрины «Идеал».
 * Если у товара есть фото — показываем его на белом канвасе (object-contain).
 * Иначе — аккуратный технический SVG-чертёж сечения (арматура, труба,
 * уголок, швеллер, лист, двутавр, мешок сыпучих) в стиле blueprints.
 */
import Image from "next/image";
import type { ReactElement } from "react";
import type { StorefrontProduct } from "@/lib/theme-storefront";

export type IdealAssetVariant =
  | "angle"
  | "pipe"
  | "rebar"
  | "channel"
...
export function resolveIdealVariant(p: StorefrontProduct): IdealAssetVariant {
  if (p.type === "BAG_30KG" || p.type === "BIG_BAG_1TON") return "bag";
  if (p.density != null) return "granule";
  const n = `${p.name} ${p.categoryName}`.toLowerCase();
  if (n.includes("уголок")) return "angle";
  if (n.includes("швеллер")) return "channel";
  if (n.includes("арматура") || n.includes("катанк")) return "rebar";
  if (n.includes("лист")) return "sheet";
  if (n.includes("труба") || n.includes("профильн") || n.includes("круг") || n.includes("прут")) return "pipe";
  if (n.includes("двутавр") || n.includes("балка") || n.includes("тавр")) return "beam";
  if (n.includes("песок") || n.includes("щебень") || n.includes("керамзит") || n.includes("грунт")) return "granule";
  return "beam";
}
...
function DimLine({
  ...
  return (
    <g>
      <line x1={s1x} y1={s1y} x2={s2x} y2={s2y} stroke="#94a3b8" strokeWidth="1" strokeDasharray="3 3" />
      ...
      <line x1={s2x} y1={s2y} x2={s2x + ux * arrow - ax * 2} y2={s2y + uy * arrow - ay * 2} stroke="#94a3b8" strokeWidth="1" />
      {label && (
        <text x={mx} y={my} textAnchor="middle" dominantBaseline="middle" fontSize="8" fontWeight="700" fill="#64748b">
          {label}
        </text>
      )}
    </g>
  ...
  return (
    <svg viewBox="0 0 140 140" className="h-full w-full" role="img" aria-label="Чертёж уголка">
      ...
      <g stroke="#3b82f6" strokeWidth="1.5" fill="none" opacity="0.5">
        <path d="M38 108 L38 38 L108 38" strokeDasharray="3 3" />
      </g>
...
function PipeDrawing() {
  return (
    <svg viewBox="0 0 140 140" className="h-full w-full" role="img" aria-label="Чертёж трубы">
      ...
      <g stroke="#3b82f6" strokeWidth="1.5" opacity="0.5" strokeDasharray="3 3">
...
function ChannelDrawing() {
  return (
    <svg viewBox="0 0 140 140" className="h-full w-full" role="img" aria-label="Чертёж швеллера">
      ...
      <g stroke="#3b82f6" strokeWidth="1.5" fill="none" opacity="0.5" strokeDasharray="3 3">
        <path d="M38 30 L102 30 L102 110" />
      </g>
...
function GranuleDrawing() {
  return (
    <svg viewBox="0 0 140 140" className="h-full w-full" role="img" aria-label="Сыпучий материал">
      <g fill="#e2e8f0" stroke="#94a3b8" strokeWidth="1">
        {Array.from({ length: 18 }, (_, i) => (
          <circle key={i} cx={30 + (i % 6) * 14} cy={34 + Math.floor(i / 6) * 34} r="5.5" />
        ))}
      </g>
      <g fill="#cbd5e1" stroke="#94a3b8" strokeWidth="1" opacity="0.6">
        {Array.from({ length: 10 }, (_, i) => (
          <circle key={i} cx={40 + (i % 5) * 13} cy={52 + Math.floor(i / 5) * 30} r="4" />
        ))}
      </g>
      <DimLine x1={30} y1={34} x2={30} y2={120} label="насыпь" offset={-16} />
    </svg>
  );
}

const DRAWINGS: Record<IdealAssetVariant, () => ReactElement> = {
  angle: AngleDrawing,
  pipe: PipeDrawing,
  rebar: RebarDrawing,
  ...
  beam: BeamDrawing,
  bag: BagDrawing,
  granule: GranuleDrawing,
};

interface IdealAssetProps {
  product: StorefrontProduct;
  className?: string;
}

export default function IdealAsset({ product, className }: IdealAssetProps) {
  const Drawing = DRAWINGS[resolveIdealVariant(product)];

  // Локальное фото (public/products/...) — безопасно для next/image,
  // внешние URL — только если приходят из админки/парсера
  const src = product.imageLocal?.startsWith("/") ? product.imageLocal : product.imageUrl;
  ...
  return (
    <div className={`flex items-center justify-center bg-gradient-to-br from-white to-slate-50 ${className ?? ""}`}>
