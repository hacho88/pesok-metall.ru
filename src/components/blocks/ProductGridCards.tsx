  ...
  return (
    <div className="block-grid">
      {products.map((p) => {
        ...
        const weightLabel =
          w > 0 ? (isLinear ? `${fmtKg(w)} кг/м` : `${fmtKg(w)} кг`) : null;
        const pricePerTon =
          isLinear && p.price != null && w > 0 ? Math.round((p.price / w) * 1000) : null;
        return (
        <div
          key={p.id}
        ...
        >
          ...
          <div className="flex flex-1 flex-col gap-2.5 p-5">
            ...
            <ul className="space-y-1 text-xs text-muted-foreground">
              ...
              {weightLabel && (
                <li>
                  Вес: <span className="font-bold text-foreground">{weightLabel}</span>
                </li>
              )}
            </ul>
            <div className="mt-auto flex items-baseline justify-between gap-2 pt-2">
              {p.isOnOrder ? (
                ...
                <div>
                  ...
                  <p className="text-[10px] font-bold text-muted-foreground">{p.unit}</p>
                </div>
              )}
              {pricePerTon != null && (
                <p className="text-right text-xs font-black text-primary">
                  {formatRubles(pricePerTon)}
                  <span className="block text-[9px] font-bold uppercase text-muted-foreground">за тонну</span>
                </p>
              )}
            </div>
